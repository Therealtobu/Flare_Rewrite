...
getgenv().Config = {
    ...
    ["Auto Chat"] = {
        ["Enabled"] = true,
        ["Content"] = {"Redz Hub Kaitun", "Redz X Hub", "Redz Hub On Top"},
        ["Time Chat"] = 300 
    },
...
L_1_["Remotes"] = {
    ["CommF_"] = game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("CommF_")
}

task.spawn(function()
    while task.wait(1) do 
        if getgenv().Config["Exit When Full Item"] then
            ...
            local hasValk = false
            local hasgc = false
            local hasrengoku = false
            local hasPole = false
            local hasSaber = false
            local hasMidnightBlade = false
            local hasDragonTrident = false
            local hasBuddySword = false
            local hasHallowScythe = false
            local hasCanvander = false
            local hasTwinHooks = false
            local hasTrueTripleKatana = false
            local hasSoulCane = false
            local hasAcidumRifle = false
            local hasKabucha = false
            local hasSerpentBow = false

            if success and type(inv) == "table" then
                for _, v in pairs(inv) do
                    if v.Name == "Godhuman" then hasGodhuman = true end
                    if v.Name == "Soul Guitar" or v.Name == "Skull Guitar" then hasSG = true end
                    if v.Name == "Cursed Dual Katana" then hasCDK = true end
                    if v.Name == "Valkyrie Helm" then hasValk = true end
                    if v.Name == "Gravity Cane" then hasgc = true end
                    if v.Name == "Rengoku" then hasrengoku = true end
                    if v.Name == "Pole" then hasPole = true end
                    if v.Name == "Saber" then hasSaber = true end
                    if v.Name == "Midnight Blade" then hasMidnightBlade = true end
                    if v.Name == "Dragon Trident" then hasDragonTrident = true end
                    if v.Name == "Buddy Sword" then hasBuddySword = true end
                    if v.Name == "Hallow Scythe" then hasHallowScythe = true end
                    if v.Name == "Canvander" then hasCanvander = true end
                    if v.Name == "Twin Hooks" then hasTwinHooks = true end
                    if v.Name == "True Triple Katana" then hasTrueTripleKatana = true end
                    if v.Name == "Soul Cane" then hasSoulCane = true end
                    if v.Name == "Acidum Rifle" then hasAcidumRifle = true end
                    if v.Name == "Kabucha" then hasKabucha = true end
                    if v.Name == "Serpent Bow" then hasSerpentBow = true end
                end
            end 

            local function checkInPlayer(name)
                return player.Backpack:FindFirstChild(name) or (player.Character and player.Character:FindFirstChild(name))
            end

            if not hasGodhuman then hasGodhuman = checkInPlayer("Godhuman") end
            if not hasSG then hasSG = (checkInPlayer("Soul Guitar") or checkInPlayer("Skull Guitar")) end
            if not hasCDK then hasCDK = checkInPlayer("Cursed Dual Katana") end
            if not hasValk then hasValk = checkInPlayer("Valkyrie Helm") end
            if not hasgc then hasgc = checkInPlayer("Gravity Cane") end
            if not hasrengoku then hasrengoku = checkInPlayer("Rengoku") end
            if not hasPole then hasPole = checkInPlayer("Pole") end
            if not hasSaber then hasSaber = checkInPlayer("Saber") end
            if not hasMidnightBlade then hasMidnightBlade = checkInPlayer("Midnight Blade") end
            if not hasDragonTrident then hasDragonTrident = checkInPlayer("Dragon Trident") end
            if not hasBuddySword then hasBuddySword = checkInPlayer("Buddy Sword") end
            if not hasHallowScythe then hasHallowScythe = checkInPlayer("Hallow Scythe") end
            if not hasCanvander then hasCanvander = checkInPlayer("Canvander") end
            if not hasTwinHooks then hasTwinHooks = checkInPlayer("Twin Hooks") end
            if not hasTrueTripleKatana then hasTrueTripleKatana = checkInPlayer("True Triple Katana") end
            if not hasSoulCane then hasSoulCane = checkInPlayer("Soul Cane") end
            if not hasAcidumRifle then hasAcidumRifle = checkInPlayer("Acidum Rifle") end
            if not hasKabucha then hasKabucha = checkInPlayer("Kabucha") end
            if not hasSerpentBow then hasSerpentBow = checkInPlayer("Serpent Bow") end
            
            
            if hasGodhuman and hasSG and hasCDK and hasValk and hasgc and hasrengoku and hasPole and hasSaber and hasMidnightBlade and hasDragonTrident and hasBuddySword and hasHallowScythe and hasCanvander and hasTwinHooks and hasTrueTripleKatana and hasSoulCane and hasAcidumRifle and hasKabucha and hasSerpentBow then
                task.spawn(function()
                    player:Kick("You already have all the items to farm.")
                    task.wait(1)
                    game:Shutdown()
                end)
                break 
...
task.spawn(function()
    ...
    while task.wait(10) do
        if getgenv().Config["HOP"]["Enabled"] then
            if tick() - startTime >= getgenv().Config["HOP"]["time hop"] then
                print("🔄 Đang tiến hành đổi server (Hop)...")
                HopServer()
                startTime = tick() 
            end
        end
...
task.spawn(function()
    while task.wait(getgenv().Config["Auto Chat"]["Time Chat"] or 30) do
        if getgenv().Config["Auto Chat"]["Enabled"] then
            local contents = getgenv().Config["Auto Chat"]["Content"]
            local msg = contents[math.random(1, #contents)]
            pcall(function()
...
task.spawn(function()
    ...
    ColorFrame.Size = UDim2.new(1, 0, 1, 100) 
    ColorFrame.Position = UDim2.new(0, 0, 0, -50)
    ColorFrame.BorderSizePixel = 0
    while task.wait(0.5) do
        local sf = getgenv().Config
        if sf["Black Screen"] or sf["White Screen"] then
            ColorFrame.Visible = true
            ColorFrame.BackgroundColor3 = sf["Black Screen"] and Color3.fromRGB(0,0,0) or Color3.fromRGB(255,255,255)
    ...
    while task.wait(5) do
        pcall(function()
            if not player.Character:FindFirstChild("HasBuso") then
                L_1_["Remotes"]["CommF_"]:InvokeServer("Buso")
            end
        end)
    end
end)
...
if not getgenv()["Configs"] then
    local cf = getgenv().Config
    getgenv()["Configs"] = {
        ["Quest"] = {
            ["Evo Race V1"] = cf.Race["Auto V2"],
            ["Evo Race V2"] = cf.Race["Auto V3"],
            ["Pull Lerver"] = true
        },
        ["Sword"] = (cf.Sword["All Sword"] and {"Saber", "Pole (1st Form)", "Rengoku", "Midnight Blade", "Soul Cane", "Yama", "Tushita", "Cursed Dual Katana"}) or {},
        ["Gun"] = (cf.Gun["All Gun"] and {"Skull Guitar", "Kabucha", "Serpent Bow"}) or {},
        ["FPS Booster"] = cf.Settings["FPS Booster"]
    }
end

-- ╔══════════════════════════════════════════════════════════════╗
-- ║       LIVE TRACKER v3  •  Redz Hub  •  Blox Fruits          ║
-- ╚══════════════════════════════════════════════════════════════╝
local _LiveTrackerLib = (function()
    local M = {}
    M.__index = M
    ...
    local guiRoot      = (function()
        local ok, cg = pcall(function() return game:GetService("CoreGui") end)
        return (ok and cg) or lp:WaitForChild("PlayerGui")
    end)()

    -- ── Palette ─────────────────────────────────────────────────
    local C_BG      = Color3.fromRGB(18, 18, 18)
    local C_WHITE   = Color3.fromRGB(255, 255, 255)
    local C_DIM     = Color3.fromRGB(140, 140, 140)
    local C_BADGE   = Color3.fromRGB(80, 200, 255)
    local C_GREEN   = Color3.fromRGB(50, 215, 75)
    local C_GREEN2  = Color3.fromRGB(30, 180, 50)
    local NEARBY_DIST = 35   -- studs để chuyển sang mode Nearby

    -- ── Tweens ──────────────────────────────────────────────────
    local TI_IN    = TweenInfo.new(0.42, Enum.EasingStyle.Back,  Enum.EasingDirection.Out)
    local TI_OUT   = TweenInfo.new(0.25, Enum.EasingStyle.Quart, Enum.EasingDirection.In)
    local TI_FAST  = TweenInfo.new(0.15)
    local TI_PULSE = TweenInfo.new(0.75, Enum.EasingStyle.Sine,  Enum.EasingDirection.InOut, -1, true)

    local POS_SHOW = UDim2.new(0.5, 0, 0, 14)
    local POS_HIDE = UDim2.new(0.5, 0, 0, -180)
    ...
    function M.new()
        ...
        self._tPart     = nil
        self._tPos      = nil
        self._nearby    = nil   -- nil = unknown, true/false
        self._pulseConn = nil
        self:_Build()
        return self
    end
    ...
    function M:_Build()
        pcall(function()
            local old = guiRoot:FindFirstChild("_RedzLT3")
        ...
        pill.Position = POS_HIDE
        pill.Size = UDim2.new(0, 230, 0, 158)
        ...
        shine.Size = UDim2.new(0.5, 0, 0, 1)
        ...
        badge.TextSize = 8
        badge.TextColor3 = C_BADGE
        badge.BackgroundTransparency = 1
        badge.AnchorPoint = Vector2.new(0, 0)
        badge.Position = UDim2.new(0, 14, 0, 13)
        badge.Size = UDim2.new(1, -80, 0, 12)
        ...
        tname.TextSize = 15
        tname.TextColor3 = C_WHITE
        tname.BackgroundTransparency = 1
        tname.AnchorPoint = Vector2.new(0, 0)
        tname.Position = UDim2.new(0, 14, 0, 26)
        tname.Size = UDim2.new(1, -30, 0, 20)
        tname.TextXAlignment = Enum.TextXAlignment.Left
        tname.TextTruncate = Enum.TextTruncate.AtEnd
        tname.Parent = pill

        -- ── Center zone ─────────────────────────────────────────
        local centerWrap = Instance.new("Frame")
        centerWrap.Name = "CenterWrap"
        centerWrap.AnchorPoint = Vector2.new(0.5, 0.5)
        centerWrap.Position = UDim2.new(0.5, 0, 0.5, 4)
        centerWrap.Size = UDim2.new(0, 64, 0, 64)
        centerWrap.BackgroundTransparency = 1
        centerWrap.Parent = pill

        -- Arrow (▲ triangle, rotates in 3D)
        local arrowLbl = Instance.new("TextLabel")
        arrowLbl.Name = "Arrow"
        arrowLbl.Text = "▲"
        arrowLbl.Font = Enum.Font.GothamBold
        arrowLbl.TextSize = 38
        arrowLbl.TextColor3 = C_WHITE
        arrowLbl.BackgroundTransparency = 1
        arrowLbl.Size = UDim2.new(1, 0, 1, 0)
        arrowLbl.AnchorPoint = Vector2.new(0.5, 0.5)
        arrowLbl.Position = UDim2.new(0.5, 0, 0.5, 0)
        arrowLbl.Rotation = 0
        arrowLbl.Parent = centerWrap

        -- Nearby dot (hidden by default)
        local nearbyDot = Instance.new("Frame")
        nearbyDot.Name = "NearbyDot"
        nearbyDot.AnchorPoint = Vector2.new(0.5, 0.5)
        nearbyDot.Position = UDim2.new(0.5, 0, 0.5, 0)
        nearbyDot.Size = UDim2.new(0, 60, 0, 60)
        nearbyDot.BackgroundColor3 = C_GREEN
        nearbyDot.BorderSizePixel = 0
        nearbyDot.Visible = false
        nearbyDot.Parent = centerWrap

        local dotCorner = Instance.new("UICorner")
        dotCorner.CornerRadius = UDim.new(1, 0)
        dotCorner.Parent = nearbyDot

        -- Inner white core of dot
        local dotCore = Instance.new("Frame")
        dotCore.AnchorPoint = Vector2.new(0.5, 0.5)
        dotCore.Position = UDim2.new(0.5, 0, 0.5, 0)
        dotCore.Size = UDim2.new(0.38, 0, 0.38, 0)
        dotCore.BackgroundColor3 = C_WHITE
        dotCore.BorderSizePixel = 0
        dotCore.Parent = nearbyDot
        local dotCoreCorner = Instance.new("UICorner")
        dotCoreCorner.CornerRadius = UDim.new(1, 0)
        dotCoreCorner.Parent = dotCore

        -- ── Bottom row ──────────────────────────────────────────
        local botRow = Instance.new("Frame")
        botRow.AnchorPoint = Vector2.new(0, 1)
        botRow.Position = UDim2.new(0, 14, 1, -13)
        botRow.Size = UDim2.new(1, -28, 0, 22)
        botRow.BackgroundTransparency = 1
        botRow.Parent = pill

        local distNum = Instance.new("TextLabel")
        distNum.Name = "DistNum"
        distNum.Text = "0"
        distNum.Font = Enum.Font.GothamBold
        distNum.TextSize = 20
        distNum.TextColor3 = C_WHITE
        distNum.BackgroundTransparency = 1
        distNum.AnchorPoint = Vector2.new(0, 0.5)
        distNum.Position = UDim2.new(0, 0, 0.5, 0)
        distNum.Size = UDim2.new(0, 72, 1, 0)
        distNum.TextXAlignment = Enum.TextXAlignment.Left
        distNum.Parent = botRow

        local distSuf = Instance.new("TextLabel")
        distSuf.Name = "DistSuf"
        distSuf.Text = "studs ahead"
        distSuf.Font = Enum.Font.Gotham
        distSuf.TextSize = 14
        distSuf.TextColor3 = C_DIM
        distSuf.BackgroundTransparency = 1
        distSuf.AnchorPoint = Vector2.new(0, 0.5)
        distSuf.Position = UDim2.new(0, 76, 0.5, 1)
        distSuf.Size = UDim2.new(1, -76, 1, 0)
        distSuf.TextXAlignment = Enum.TextXAlignment.Left
        distSuf.Parent = botRow
        ...
        xBtn.TextSize = 11
        xBtn.TextColor3 = C_DIM
        xBtn.BackgroundTransparency = 1
        xBtn.AnchorPoint = Vector2.new(1, 0)
        xBtn.Position = UDim2.new(1, -9, 0, 9)
        xBtn.Size = UDim2.new(0, 20, 0, 20)
        xBtn.Parent = pill
        xBtn.MouseButton1Click:Connect(function() self:Hide() end)

        -- Store refs
        self._sg        = sg
        self._pill      = pill
        self._tLbl      = tname
        self._arrow     = arrowLbl
        self._nearbyDot = nearbyDot
        self._distNum   = distNum
        self._distSuf   = distSuf
    end

    -- ── Public API ──────────────────────────────────────────────

    function M:Show(name, target)
        if not self._sg or not self._sg.Parent then self:_Build() end
        -- Flash-update name
        if self._active and self._tLbl.Text ~= (name or "") then
            TweenService:Create(self._tLbl, TI_FAST, {TextTransparency=1}):Play()
            task.delay(0.12, function()
                if self._tLbl then
                    self._tLbl.Text = name or "..."
                    TweenService:Create(self._tLbl, TI_FAST, {TextTransparency=0}):Play()
                end
            end)
        else
            self._tLbl.Text = name or "..."
        ...
        if not self._active then
            self._active = true
            self._nearby = nil
            self._pill.Position = POS_HIDE
            TweenService:Create(self._pill, TI_IN, {Position=POS_SHOW}):Play()
            self:_StartLoop()
        end
    end
    ...
    function M:Hide()
        ...
        self:_StopPulse()
        if self._pill then
            TweenService:Create(self._pill, TI_OUT, {Position=POS_HIDE}):Play()
        end
    end

    -- ── Internal ────────────────────────────────────────────────

    function M:_SetTarget(t)
        if typeof(t) == "Vector3" then
            self._tPos = t; self._tPart = nil
        elseif typeof(t) == "Instance" then
            self._tPart = (t:IsA("Model") and (t.PrimaryPart or t:FindFirstChildWhichIsA("BasePart"))) or t
            self._tPos = nil
        else
            self._tPos = nil; self._tPart = nil
        end
    end

    function M:_GetTP()
        if self._tPart and self._tPart.Parent then return self._tPart.Position end
        return self._tPos
    end

    function M:_StartLoop()
        self:_StopLoop()
    ...
    function M:_StopLoop()
        if self._conn then self._conn:Disconnect(); self._conn = nil end
    end

    function M:_StartPulse()
        self:_StopPulse()
        if not self._nearbyDot then return end
        local dot = self._nearbyDot
        TweenService:Create(dot, TI_PULSE, {
            Size = UDim2.new(0, 52, 0, 52),
            BackgroundTransparency = 0.3,
        }):Play()
    end

    function M:_StopPulse()
        if self._nearbyDot then
            TweenService:Create(self._nearbyDot, TweenInfo.new(0.2), {
    ...
    function M:_SetNearbyMode(nearby)
        if self._nearby == nearby then return end
        self._nearby = nearby
        if nearby then
            -- Switch to green dot mode
            TweenService:Create(self._arrow, TI_FAST, {TextTransparency=1}):Play()
            task.delay(0.12, function()
                if self._nearbyDot then
                    self._nearbyDot.Visible = true
                    self._nearbyDot.Size = UDim2.new(0, 60, 0, 60)
                    self:_StartPulse()
                end
            end)
            -- Bottom text: "Nearby" in green
            self._distNum.Text = "Nearby"
            self._distNum.TextColor3 = C_GREEN
            self._distNum.TextSize = 16
            self._distSuf.Text = ""
        else
            -- Switch to arrow mode
            if self._nearbyDot then
                self:_StopPulse()
                self._nearbyDot.Visible = false
            end
            TweenService:Create(self._arrow, TI_FAST, {TextTransparency=0}):Play()
            self._distNum.TextColor3 = C_WHITE
            self._distNum.TextSize = 20
        end
    end
    ...
    function M:_Tick()
        ...
        local dist = math.round(diff.Magnitude)

        -- ── Nearby mode ─────────────────────────────────────────
        if dist <= NEARBY_DIST then
            self:_SetNearbyMode(true)
            return
        end
        self:_SetNearbyMode(false)

        -- ── Distance text ────────────────────────────────────────
        self._distNum.Text = tostring(dist)

        -- ── 3D arrow: project full direction onto camera view ────
        -- Uses camera RightVector + UpVector for full 3D freedom
        local cam  = workspace.CurrentCamera
        local camR = cam.CFrame.RightVector
        local camU = cam.CFrame.UpVector
        local camF = -cam.CFrame.LookVector  -- forward is -LookVector

        -- Normalize direction to target
        local dir = diff.Unit

        -- Camera-space components
        local screenX = dir:Dot(camR)   -- left/right on screen
        local screenY = dir:Dot(camU)   -- up/down on screen
        local depthZ  = dir:Dot(camF)   -- depth (positive = in front)

        -- atan2 gives angle from screen-up, rotating clockwise
        -- screenX positive = right, screenY positive = up
        local angleDeg = math.deg(math.atan2(screenX, screenY))
        self._arrow.Rotation = angleDeg

        -- Suffix based on depth
        self._distSuf.Text = (depthZ < 0) and "studs ahead" or "studs behind"
    end

    return M
end)()

local _RedzTracker = _LiveTrackerLib.new()
getgenv()._RedzTracker = _RedzTracker

getgenv()._QuestMap = {
    ["Saber"]               = Vector3.new(1144.5,   7.3,   -1164.7),
    ["Shark Saw"]           = Vector3.new(-1836,    11,     1714),
    ["Wardens Sword"]       = Vector3.new(-1458.9,  29.9,  -50.6),
    ["Pole (1st Form)"]     = Vector3.new(976.5,    111.2,  1229.1),
    ["Bazooka"]             = Vector3.new(976.5,    111.2,  1229.1),
    ["Magma Blaster"]       = Vector3.new(-1935,    6,     -2564),
    ["BartiloQuest"]        = Vector3.new(1347.3,   37.3,  -1325.4),
    ["Find Water Key"]      = Vector3.new(1347.3,   37.3,  -1325.4),
    ["Rengoku"]             = Vector3.new(-5561.1,  314.4, -2663.9),
    ["Flail"]               = Vector3.new(-3025.3,  66.8,  -2937.6),
    ["Longsword"]           = Vector3.new(-3025.3,  66.8,  -2937.6),
    ["Gravity Blade"]       = Vector3.new(-2091.9,  70.0,  -12142.8),
    ["Don Swan"]            = Vector3.new(-1935,    6,     -2564),
    ["Evo Race V1"]         = Vector3.new(1347.3,   37.3,  -1325.4),
    ["Evo Race V2"]         = Vector3.new(-5561.1,  314.4, -2663.9),
    ["Yama"]                = Vector3.new(-9505.9,  172.1,  6158.9),
    ["Longma"]              = Vector3.new(-12379.1, 601.4, -6543.6),
    ["Soul Guitar"]         = Vector3.new(-2286.7,  146.6, -12226.9),
    ["RGB"]                 = Vector3.new(-9505.9,  172.1,  6158.9),
    ["Venom Bow"]           = Vector3.new(-12379.1, 601.4, -6543.6),
    ["Twin Hooks"]          = Vector3.new(-10147.8, 138.6,  5939.6),
    ["Pull Lerver"]         = Vector3.new(-12379.1, 601.4, -6543.6),
    ["Cursed Dual Katana"]  = Vector3.new(-5034,    315,   -2951),
    ["Quest Electric Claw"] = Vector3.new(-9505.9,  172.1,  6158.9),
    ["Godhuman"]            = Vector3.new(-5034,    315,   -2951),
}
-- ╚══════════════════════════════════════════════════════════════╝
...
game.StarterGui:SetCore("SendNotification", {
    Title = "Redz X Hub",
    Text = "Loading script kaitun freemium...",
    Icon = "rbxassetid://138697314212751",
    Duration = 10,
})
...
L_1_[42]["Size"] = UDim2.new(0, 350, 0, 60)
...
L_1_[26]["Text"] = "Status : Redz Hub Kaitun"
...
-- ── Hook: ẩn board cũ + bắt status text → show tracker ──────
do
    L_1_[42].Visible = false  -- ẩn frame cũ

    local function _onStatus(rawText)
        local txt = rawText:gsub("^%s*Status%s*:%s*", ""):gsub("^%s+",""):gsub("%s+$","")
        if txt == "" or txt == "Redz Hub Kaitun" then return end
        local pos = (getgenv()._QuestMap or {})[getgenv().Quest or ""] or Vector3.new(0,0,0)
        getgenv()._RedzTracker:Show(txt, pos)
    end

    L_1_[26]:GetPropertyChangedSignal("Text"):Connect(function()
        pcall(_onStatus, L_1_[26].Text)
    end)

    task.spawn(function()
        local lastQ = getgenv().Quest
        while task.wait(1) do
            local q = getgenv().Quest
            if q ~= lastQ then
                lastQ = q
                if not q or q == "" then
                    getgenv()._RedzTracker:Hide()
                else
                    local pos = (getgenv()._QuestMap or {})[q]
                    if pos and getgenv()._RedzTracker._active then
                        getgenv()._RedzTracker:_SetTarget(pos)
                    end
                end
            end
        end
...
task.spawn(function()
	while task.wait() do
		...
		local T1 = L_1_[6]:Create(L_1_[12], info, {Color = Color3.fromRGB(150, 0, 0)})
		local T2 = L_1_[6]:Create(L_1_[12], info, {Color = Color3.fromRGB(150, 0, 0)})
		local T3 = L_1_[6]:Create(L_1_[15], info, {TextColor3 = Color3.fromRGB(150, 0, 0)})
		local T4 = L_1_[6]:Create(L_1_[15], info, {TextColor3 = Color3.fromRGB(150, 0, 0)})
		local T5 = L_1_[6]:Create(L_1_[26], info, {TextColor3 = Color3.fromRGB(150, 0, 0)})
		local T6 = L_1_[6]:Create(L_1_[26], info, {TextColor3 = Color3.fromRGB(150, 0, 0)})

		T1:Play(); T3:Play(); T5:Play()
		T1.Completed:Wait()
		T2:Play(); T4:Play(); T6:Play()
		T2.Completed:Wait()
	end
end)
...
local TweenService = game:GetService("TweenService")
local OldGUIs = {CoreGui:FindFirstChild("Status"), CoreGui:FindFirstChild("Redz Hub Btn"), CoreGui:FindFirstChild("CoinCard")}
...
blur.Name = "Redz Hub Blur"
blur.Parent = Lighting
blur.Size = getgenv().SettingFarm["Hide UI"] and 0 or 24
...
UIStrokeMain.Color = Color3.fromRGB(150, 0, 0)
...
TopTitle.Text = "Redz Checker Stats"
TopTitle.TextColor3 = Color3.fromRGB(150, 0, 0)
...
UnderStats.TextColor3 = Color3.fromRGB(150, 0, 0)
...
UnderItems.TextColor3 = Color3.fromRGB(150, 0, 0)
UnderItems.TextSize = 16
UnderItems.FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold)

local RedzHubBtn = Instance.new("ScreenGui")
local dutdit = Instance.new("Frame")
local UICornerBtn = Instance.new("UICorner")
local ImageLabel = Instance.new("ImageLabel")
local TextButton = Instance.new("TextButton")

RedzHubBtn.Name = "Redz Hub Btn"  
RedzHubBtn.Parent = game:GetService("CoreGui")
RedzHubBtn.ZIndexBehavior = Enum.ZIndexBehavior.Sibling  
RedzHubBtn.DisplayOrder = 10
RedzHubBtn.ResetOnSpawn = false
if getgenv().SettingFarm["Hide UI"] then
    RedzHubBtn.Enabled = false
end

dutdit.Name = "dut dit"  
dutdit.Parent = RedzHubBtn  
...
ImageLabel.Image = "rbxassetid://138697314212751"  
...
local function SyncItems()
    local success, inventory = pcall(function() return ReplicatedStorage.Remotes.CommF_:InvokeServer("getInventory") end)
...
task.spawn(function()
    ...
    while task.wait(2) do
        if CoinCard.Enabled then
            ...
            local hasValk = inv["Valkyrie Helm"] or (player.Character and player.Character:FindFirstChild("Valkyrie Helm"))
            ValkyrieHelmLabel.Text = (hasValk and ICON_GREEN or ICON_RED) .. " Valkyrie Helm"

            local hasCDK = inv["Cursed Dual Katana"] or player.Backpack:FindFirstChild("Cursed Dual Katana") or (player.Character and player.Character:FindFirstChild("Cursed Dual Katana"))
            ...
            local hasSG = inv["Skull Guitar"] or player.Backpack:FindFirstChild("Skull Guitar") or (player.Character and player.Character:FindFirstChild("Skull Guitar"))
            ...
            LevelLabel.Text = "Level: " .. player.Data.Level.Value .. "   Third Sea: " .. (hasBadge and ICON_OK or ICON_X)
            BeliLabel.Text = "Beli: " .. player.Data.Beli.Value
            FragLabel.Text = "Frag: " .. player.Data.Fragments.Value
            RaceLabel.Text = "Race: " .. player.Data.Race.Value
        end
    end
end)
-- World check
	...
	while L_1_[45]["wt"](1) do
		pcall(function()
			if not L_1_[45]["ffc"](L_1_[35]["Character"], "Highlight") then
				...
				L_646_[1]["OutlineColor"] = Color3["fromRGB"](180, 0, 0)
