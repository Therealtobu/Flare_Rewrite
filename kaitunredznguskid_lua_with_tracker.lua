...
-- ╔════════════════════════════════════════════════════════════╗
-- ║              LIVE TRACKER MODULE (Redz Hub)               ║
-- ║         Apple Dynamic Island style • Blox Fruits          ║
-- ╚════════════════════════════════════════════════════════════╝
local _LiveTrackerLib = (function()
    ...
    local COLOR_BG  = Color3.fromRGB(16, 16, 16)
    local COLOR_W   = Color3.fromRGB(255, 255, 255)
    local COLOR_DIM = Color3.fromRGB(150, 150, 150)
    local COLOR_AC  = Color3.fromRGB(80, 200, 255)
    local AI = TweenInfo.new(0.45, Enum.EasingStyle.Back,  Enum.EasingDirection.Out)
    local AO = TweenInfo.new(0.28, Enum.EasingStyle.Quart, Enum.EasingDirection.In)
    ...
    function M:_Build()
        pcall(function()
            local old = guiRoot:FindFirstChild("_RedzLiveTracker")
            if old then old:Destroy() end
        end)

        local sg = Instance.new("ScreenGui")
        sg.Name           = "_RedzLiveTracker"
        ...
        pill.Size             = UDim2.new(0, 228, 0, 152)
        pill.BackgroundColor3 = COLOR_BG
        ...
        badge.TextSize = 8.5
        badge.TextColor3 = COLOR_AC
        badge.BackgroundTransparency = 1
        badge.AnchorPoint = Vector2.new(0, 0)
        badge.Position = UDim2.new(0, 15, 0, 13)
        ...
        tname.TextSize = 17
        tname.TextColor3 = COLOR_W
        tname.BackgroundTransparency = 1
        tname.AnchorPoint = Vector2.new(0, 0)
        tname.Position = UDim2.new(0, 15, 0, 27)
        ...
        aWrap.AnchorPoint = Vector2.new(0.5, 0.5)
        aWrap.Position = UDim2.new(0.5, 0, 0.5, 6)
        aWrap.Size = UDim2.new(0, 50, 0, 50)
        aWrap.BackgroundTransparency = 1
        aWrap.Parent = pill
        ...
        arrow.Image = "rbxassetid://6034509993"  -- built-in white up-arrow
        arrow.ImageColor3 = COLOR_W
        arrow.ScaleType = Enum.ScaleType.Fit
        arrow.Parent = aWrap

        -- Distance row
        local dRow = Instance.new("Frame")
        dRow.AnchorPoint = Vector2.new(0, 1)
        dRow.Position = UDim2.new(0, 15, 1, -13)
        dRow.Size = UDim2.new(1, -30, 0, 22)
        ...
        dNum.Font = FONT_BOLD
        dNum.TextSize = 19
        dNum.TextColor3 = COLOR_W
        dNum.BackgroundTransparency = 1
        dNum.AnchorPoint = Vector2.new(0, 0.5)
        dNum.Position = UDim2.new(0, 0, 0.5, 0)
        dNum.Size = UDim2.new(0, 68, 1, 0)
        dNum.TextXAlignment = Enum.TextXAlignment.Left
        dNum.Parent = dRow

        local dSuf = Instance.new("TextLabel")
        dSuf.Name = "DistSuffix"
        dSuf.Text = "studs ahead"
        dSuf.Font = FONT_REG
        dSuf.TextSize = 14
        dSuf.TextColor3 = COLOR_DIM
        dSuf.BackgroundTransparency = 1
        dSuf.AnchorPoint = Vector2.new(0, 0.5)
        dSuf.Position = UDim2.new(0, 72, 0.5, 1)
        dSuf.Size = UDim2.new(1, -72, 1, 0)
        dSuf.TextXAlignment = Enum.TextXAlignment.Left
        dSuf.Parent = dRow
    ...
    function M:Show(name, target)
        ...
        TweenService:Create(self._pill, AI, { Position = POS_SHOW }):Play()
        self:_StartLoop()
    end

    function M:UpdateTarget(name, target)
        if self._tLbl then
            TweenService:Create(self._tLbl, TweenInfo.new(0.12), { TextTransparency = 1 }):Play()
            task.delay(0.12, function()
                if self._tLbl then
                    self._tLbl.Text = name or "Unknown"
                    TweenService:Create(self._tLbl, TweenInfo.new(0.18), { TextTransparency = 0 }):Play()
                end
            end)
        end
        self:_SetTarget(target)
    ...
    function M:Hide()
        ...
        self:_StopLoop()
        if self._pill then
            TweenService:Create(self._pill, AO, { Position = POS_HIDE }):Play()
        end
    end
    ...
    function M:_SetTarget(t)
        ...
        elseif typeof(t) == "Instance" then
            if t:IsA("Model") then
                self._tPart = t.PrimaryPart or t:FindFirstChildWhichIsA("BasePart")
            else
                self._tPart = t
            end
            self._tPos = nil
    ...
    function M:_Tick()
        ...
        -- Arrow direction (camera-relative, horizontal plane)
        local cam    = workspace.CurrentCamera
        local toVec  = Vector3.new((tp - hrp.Position).X, 0, (tp - hrp.Position).Z)
        if toVec.Magnitude < 0.001 then return end
        toVec = toVec.Unit

        local cfL = cam.CFrame.LookVector
        local cfR = cam.CFrame.RightVector
        local fL  = Vector3.new(cfL.X, 0, cfL.Z)
        local fR  = Vector3.new(cfR.X, 0, cfR.Z)
        if fL.Magnitude < 0.001 then return end
        fL = fL.Unit; fR = fR.Unit

        local fwd   = toVec:Dot(-fL)
        local right = toVec:Dot(fR)
...
-- ─── Tạo instance tracker ──────────────────────────────────────
local _RedzTracker = _LiveTrackerLib.new()
getgenv()._RedzTracker = _RedzTracker

-- ─── Map Quest → toạ độ đảo (Blox Fruits) ─────────────────────
-- Lấy trực tiếp từ các CFrame teleport trong script chính
local _QuestPositionMap = {
    -- ═══ First Sea ═══
    ["Saber"]           = Vector3.new(1144.5,   7.3,   -1164.7),  -- Saber Expert Island
    ["Shark Saw"]       = Vector3.new(-1836,     11,     1714),    -- Fishman Island
    ["Wardens Sword"]   = Vector3.new(-1458.9,   29.9,   -50.6),   -- Prison Island
    ["Pole (1st Form)"] = Vector3.new(976.5,     111.2,  1229.1),  -- Skylands
    ["Bazooka"]         = Vector3.new(976.5,     111.2,  1229.1),  -- Sky Island boss
    ["Magma Blaster"]   = Vector3.new(-1935,     6,      -2564),   -- Magma Village
    ["BartiloQuest"]    = Vector3.new(1347.3,    37.3,   -1325.4), -- Kingdom of Rose
    ["Find Water Key"]  = Vector3.new(1347.3,    37.3,   -1325.4), -- Kingdom of Rose
    -- ═══ Second Sea ═══
    ["Rengoku"]         = Vector3.new(-5561.1,   314.4,  -2663.9), -- Ice Castle
    ["Flail"]           = Vector3.new(-3025.3,   66.8,   -2937.6), -- Green Zone
    ["Longsword"]       = Vector3.new(-3025.3,   66.8,   -2937.6), -- Green Zone (Diamond)
    ["Gravity Blade"]   = Vector3.new(-4048,     70,     -12142),  -- Underwater City
    ["Don Swan"]        = Vector3.new(-1935,     6,      -2564),   -- Haunted Castle
    ["Evo Race V1"]     = Vector3.new(1347.3,    37.3,   -1325.4), -- Kingdom of Rose
    ["Evo Race V2"]     = Vector3.new(-5561.1,   314.4,  -2663.9), -- Ice Castle area
    -- ═══ Third Sea ═══
    ["Yama"]            = Vector3.new(-9505.9,   172.1,  6158.9),  -- Port Town / Floating Turtle
    ["Longma"]          = Vector3.new(-12379.1,  601.4,  -6543.6), -- Floating Turtle area
    ["Soul Guitar"]     = Vector3.new(-2286.7,   146.6,  -12226.9),-- Sea of Treats
    ["RGB"]             = Vector3.new(-9505.9,   172.1,  6158.9),  -- Port Town
    ["Venom Bow"]       = Vector3.new(-12379.1,  601.4,  -6543.6), -- Hydra Island area
    ["Twin Hooks"]      = Vector3.new(-10147.8,  138.6,  5939.6),  -- Sea of Treats (Captain Elephant)
    ["Pull Lerver"]     = Vector3.new(-12379.1,  601.4,  -6543.6), -- Mystic Island
    ["Cursed Dual Katana"] = Vector3.new(-5034,  315,    -2951),   -- Boat Castle / Haunted Castle
    ["Quest Electric Claw"] = Vector3.new(-9505.9, 172.1, 6158.9),
    ["Godhuman"]        = Vector3.new(-5034,     315,    -2951),
}

-- ─── Watcher: tự động cập nhật khi Quest thay đổi ─────────────
local _lastTrackedQuest = nil
task.spawn(function()
    while task.wait(0.5) do
        pcall(function()
            local q = getgenv().Quest
            if q ~= _lastTrackedQuest then
                _lastTrackedQuest = q
                if q and q ~= "" then
                    local pos = _QuestPositionMap[q]
                    local label = "Target: " .. tostring(q)
                    if pos then
                        _RedzTracker:Show(label, pos)
                    else
                        -- Quest chưa có map → vẫn hiện tên, arrow ko quay
                        _RedzTracker:Show(label, Vector3.new(0, 0, 0))
                    end
                else
                    _RedzTracker:Hide()
                end
...
print("[LiveTracker] ✓ Redz Hub tracker loaded")
-- ╚════════════════════════════════════════════════════════════╝
