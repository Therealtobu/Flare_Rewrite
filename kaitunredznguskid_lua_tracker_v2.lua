...
-- ╔════════════════════════════════════════════════════════════╗
-- ║        LIVE TRACKER MODULE  •  Redz Hub  •  Blox Fruits   ║
-- ╚════════════════════════════════════════════════════════════╝
local _LiveTrackerLib = (function()
    ...
    local guiRoot      = (pcall(function() return game:GetService("CoreGui") end))
                         and game:GetService("CoreGui") or lp:WaitForChild("PlayerGui")
    local FB = Enum.Font.GothamBold
    local FR = Enum.Font.Gotham
    local CBG = Color3.fromRGB(16, 16, 16)
    local CW  = Color3.fromRGB(255, 255, 255)
    local CD  = Color3.fromRGB(150, 150, 150)
    local CAC = Color3.fromRGB(80, 200, 255)
    local AI  = TweenInfo.new(0.42, Enum.EasingStyle.Back,  Enum.EasingDirection.Out)
    local AO  = TweenInfo.new(0.25, Enum.EasingStyle.Quart, Enum.EasingDirection.In)
    local PS  = UDim2.new(0.5, 0, 0, 14)
    local PH  = UDim2.new(0.5, 0, 0, -180)

    function M.new()
        local self = setmetatable({}, M)
        self._active = false; self._conn = nil
        self._tPart = nil;    self._tPos = nil
        self:_Build()
        return self
    end
    ...
    function M:_Build()
        pcall(function()
            local old = guiRoot:FindFirstChild("_RedzLT")
            if old then old:Destroy() end
        end)
        local sg = Instance.new("ScreenGui")
        sg.Name="_RedzLT"; sg.ResetOnSpawn=false
        sg.ZIndexBehavior=Enum.ZIndexBehavior.Sibling
        sg.DisplayOrder=999; sg.IgnoreGuiInset=true; sg.Parent=guiRoot

        local pill = Instance.new("Frame")
        pill.Name="Pill"; pill.AnchorPoint=Vector2.new(0.5,0)
        pill.Position=PH; pill.Size=UDim2.new(0,228,0,152)
        pill.BackgroundColor3=CBG; pill.BackgroundTransparency=0.06
        pill.BorderSizePixel=0; pill.ClipsDescendants=true; pill.Parent=sg
        local cv=Instance.new("UICorner"); cv.CornerRadius=UDim.new(0,24); cv.Parent=pill
        local sv=Instance.new("UIStroke"); sv.Color=Color3.fromRGB(65,65,65)
        sv.Transparency=0.45; sv.Thickness=1.2; sv.Parent=pill
        local sh=Instance.new("Frame"); sh.Size=UDim2.new(0.55,0,0,1)
        sh.AnchorPoint=Vector2.new(0.5,0); sh.Position=UDim2.new(0.5,0,0,0)
        sh.BackgroundColor3=CW; sh.BackgroundTransparency=0.55
        sh.BorderSizePixel=0; sh.Parent=pill

        local badge=Instance.new("TextLabel")
        badge.Text="TRACKING"; badge.Font=FB; badge.TextSize=8.5
        badge.TextColor3=CAC; badge.BackgroundTransparency=1
        badge.AnchorPoint=Vector2.new(0,0); badge.Position=UDim2.new(0,15,0,13)
        badge.Size=UDim2.new(1,-30,0,13)
        badge.TextXAlignment=Enum.TextXAlignment.Left; badge.Parent=pill

        local tname=Instance.new("TextLabel")
        tname.Name="TargetName"; tname.Text="Loading..."; tname.Font=FB
        tname.TextSize=15; tname.TextColor3=CW; tname.BackgroundTransparency=1
        tname.AnchorPoint=Vector2.new(0,0); tname.Position=UDim2.new(0,15,0,27)
        tname.Size=UDim2.new(1,-30,0,22)
        tname.TextXAlignment=Enum.TextXAlignment.Left
        tname.TextTruncate=Enum.TextTruncate.AtEnd; tname.Parent=pill

        local aWrap=Instance.new("Frame")
        aWrap.AnchorPoint=Vector2.new(0.5,0.5); aWrap.Position=UDim2.new(0.5,0,0.5,6)
        aWrap.Size=UDim2.new(0,50,0,50); aWrap.BackgroundTransparency=1; aWrap.Parent=pill
        local arrow=Instance.new("ImageLabel")
        arrow.Name="Arrow"; arrow.Size=UDim2.new(1,0,1,0)
        arrow.BackgroundTransparency=1; arrow.Image="rbxassetid://6034509993"
        arrow.ImageColor3=CW; arrow.ScaleType=Enum.ScaleType.Fit; arrow.Parent=aWrap

        local dRow=Instance.new("Frame")
        dRow.AnchorPoint=Vector2.new(0,1); dRow.Position=UDim2.new(0,15,1,-13)
        dRow.Size=UDim2.new(1,-30,0,22); dRow.BackgroundTransparency=1; dRow.Parent=pill
        local dNum=Instance.new("TextLabel")
        dNum.Name="DistNum"; dNum.Text="0"; dNum.Font=FB; dNum.TextSize=19
        dNum.TextColor3=CW; dNum.BackgroundTransparency=1
        dNum.AnchorPoint=Vector2.new(0,0.5); dNum.Position=UDim2.new(0,0,0.5,0)
        dNum.Size=UDim2.new(0,68,1,0)
        dNum.TextXAlignment=Enum.TextXAlignment.Left; dNum.Parent=dRow
        local dSuf=Instance.new("TextLabel")
        dSuf.Name="DistSuffix"; dSuf.Text="studs"; dSuf.Font=FR; dSuf.TextSize=14
        dSuf.TextColor3=CD; dSuf.BackgroundTransparency=1
        dSuf.AnchorPoint=Vector2.new(0,0.5); dSuf.Position=UDim2.new(0,72,0.5,1)
        dSuf.Size=UDim2.new(1,-72,1,0)
        dSuf.TextXAlignment=Enum.TextXAlignment.Left; dSuf.Parent=dRow

        local xBtn=Instance.new("TextButton")
        xBtn.Text="x"; xBtn.Font=FB; xBtn.TextSize=11; xBtn.TextColor3=CD
        xBtn.BackgroundTransparency=1; xBtn.AnchorPoint=Vector2.new(1,0)
        xBtn.Position=UDim2.new(1,-9,0,9); xBtn.Size=UDim2.new(0,20,0,20); xBtn.Parent=pill
        xBtn.MouseButton1Click:Connect(function() self:Hide() end)

        self._sg=sg; self._pill=pill; self._tLbl=tname
        self._arrow=arrow; self._dNum=dNum; self._dSuf=dSuf
    end
    ...
    function M:Show(name, target)
        ...
        if self._active and self._tLbl.Text ~= (name or "") then
            TweenService:Create(self._tLbl,TweenInfo.new(0.1),{TextTransparency=1}):Play()
            task.delay(0.1, function()
                if self._tLbl then
                    self._tLbl.Text = name or "Unknown"
                    TweenService:Create(self._tLbl,TweenInfo.new(0.15),{TextTransparency=0}):Play()
                end
            end)
        else
            self._tLbl.Text = name or "Unknown"
        ...
        if not self._active then
            self._active = true
            self._pill.Position = PH
            TweenService:Create(self._pill, AI, {Position=PS}):Play()
            self:_StartLoop()
        end
    end

    function M:Hide()
        self._active = false; self:_StopLoop()
        if self._pill then TweenService:Create(self._pill,AO,{Position=PH}):Play() end
    end
    ...
    function M:_SetTarget(t)
        ...
        elseif typeof(t)=="Instance" then
            ...
            self._tPos=nil
        else self._tPos=nil; self._tPart=nil end
    end
    ...
    function M:_Tick()
        ...
        if not tp then self._dNum.Text="--"; return end
        local dist = math.round((hrp.Position - tp).Magnitude)
        self._dNum.Text = tostring(dist)
        local cam = workspace.CurrentCamera
        local toV = Vector3.new((tp-hrp.Position).X,0,(tp-hrp.Position).Z)
        if toV.Magnitude < 0.001 then return end
        toV=toV.Unit
        local fL=cam.CFrame.LookVector; local fR=cam.CFrame.RightVector
        local fLf=Vector3.new(fL.X,0,fL.Z); local fRf=Vector3.new(fR.X,0,fR.Z)
        if fLf.Magnitude<0.001 then return end
        fLf=fLf.Unit; fRf=fRf.Unit
        local fwd=toV:Dot(-fLf); local right=toV:Dot(fRf)
        self._arrow.Rotation=math.deg(math.atan2(right,fwd))
        self._dSuf.Text=(fwd>=0) and "studs ahead" or "studs behind"
    end
...
do
    L_1_[42].Visible = false  -- ẩn frame cũ "Redz Hub - Kaitun"
